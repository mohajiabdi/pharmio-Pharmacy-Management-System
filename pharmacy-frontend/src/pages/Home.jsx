...
export default function Home() {
  ...
  // apply theme on load: localStorage > system
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
      ...
      <header className="sticky top-0 z-40 border-b border-[hsl(var(--border))] bg-[hsl(var(--background)/0.8)] backdrop-blur">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-5 py-4">
          ...
          <nav className="hidden items-center gap-6 lg:flex">
            <a
              href="#features"
              className="text-sm font-extrabold text-[hsl(var(--foreground)/0.7)] hover:text-[hsl(var(--foreground))]"
            >
              Features
            </a>
            <a
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 md:items-center">
          <div>
            ...
            <p className="mt-3 text-sm font-medium text-[hsl(var(--foreground)/0.65)] md:text-base">
              Track medicines, stock, sales, customers, suppliers, and reports —
              all in one place. Designed for daily pharmacy operations with
              speed and clarity.
            </p>
          ...
          <div className="rounded-3xl border border-[hsl(var(--border))] bg-[hsl(var(--surface))] p-5 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <MiniKPI title="Revenue" value="$1,240" />
              <MiniKPI title="Orders" value="36" />
              <MiniKPI title="Low Stock" value="8" />
              <MiniKPI title="Customers" value="21" />
            </div>
            ...
            <div className="mt-4 rounded-2xl border border-[hsl(var(--border))] bg-[hsl(var(--background))] p-4">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                <ActionPill icon={Users} text="New Customer" />
                <ActionPill icon={Truck} text="Add Supplier" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="features" className="mx-auto w-full max-w-6xl px-5 pb-12">
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
          ...
          <Card
            icon={Users}
            title="Customers"
            desc="Store customer details and connect sales to customers."
          />
          <Card
            icon={Truck}
            title="Suppliers"
            desc="Save supplier contacts and prepare for purchases/restocking."
          />
          <Card
            icon={BarChart3}
            title="Reports"
            desc="Revenue, orders, top products, and payment breakdown."
          />
          <Card
            icon={ShieldCheck}
            title="Secure Access"
      ...
      {/* Pricing */}
      <section id="pricing" className="mx-auto w-full max-w-6xl px-5 pb-12">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          <Pricing
            title="Starter"
            price="$0"
            note="Demo / Student project"
            features={["All modules", "Local storage data", "Basic reports"]}
          />
          <Pricing
            title="Business"
            price="$29"
          ...
          <Pricing
            ...
            features={["Branch support", "Audit logs", "Custom integrations"]}
          />
        </div>
      </section>
...
function FeatureDetail({ icon: Icon, title, points }) {
  return (
    <div className="rounded-3xl border border-[hsl(var(--border))] bg-[hsl(var(--surface))] p-6 shadow-sm">
      <div className="flex items-start gap-3">
        <div className="grid h-12 w-12 place-items-center rounded-2xl bg-[hsl(var(--brand-strong)/0.15)]">
          <Icon className="h-6 w-6 text-[hsl(var(--brand-strong))]" />
        </div>
        <div className="min-w-0">
          ...
          <ul className="mt-3 space-y-2">
            {points.map((p) => (
              <li
                key={p}
                className="flex items-start gap-2 text-sm font-semibold text-[hsl(var(--foreground)/0.75)]"
              >
                <span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[hsl(var(--brand-strong))]" />
                <span>{p}</span>
              </li>
            ))}
          </ul>
...
function HeaderActions({ isDark, toggleTheme }) {
  const [open, setOpen] = useState(false);

  // close on resize up to desktop (optional but clean)
  useEffect(() => {
    const onResize = () => {
